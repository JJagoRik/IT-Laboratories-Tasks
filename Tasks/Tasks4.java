...
public class Tasks4{
    public static String composition(int n, int k, String str){                                                         // 1 задание
        if (str.split(" ").length > k) return "Слов больше, чем по условию";
    ...
    public static Object[] split(String string){                                                                        // 2 задание
        int start = 0;
        int end = 0;
        ArrayList<String> fin = new ArrayList<>();
        int end_index = 0;
        for (int i = 0; i < string.length(); i++) {
            if (string.charAt(i) == '(') start += 1;
            else if (string.charAt(i) == ')') end += 1;
            if (start == end) {
                fin.add(string.substring(end_index, i + 1));
                end_index = i+1;
            }
        }
        return fin.toArray();
    ...
    public static String toCamelCase(String string){                                                                    // 3 задание
    ...
    public static String toSnakeCase(String string){
        StringBuilder fin = new StringBuilder(string);
        for (int i = 0; i < fin.length(); i++) {
            if (Character.toString(fin.charAt(i)).matches("[A-Z]")) fin.replace(i, i+1, "_" +
                    Character.toString(fin.charAt(i)).toLowerCase());
        }
        return fin.toString();
    }

    public static String overTime(double st_time, double end_time, double m_h, double coef){                            // 4 задание
        if (end_time <= 17){
    ...
    public static String BMI(String weight, String height){                                                             // 5 задание
        double w = 0;
        double h = 0;
    ...
    public static int bugger(int num){                                                                                  // 6 задание
        if (num < 10) return 0;
    ...
    public static String toStarShorthand(String string){                                                                // 7 задание
        StringBuilder fin = new StringBuilder(string);
        int len = 1;
        int st_index = 0;
        int en_index = 0;
    ...
    public static boolean doesRhyme(String str1, String str2){                                                          // 8 задание
    ...
    public static boolean trouble(int num1, int num2){                                                                  // 9 задание
    ...
    public static int countUniqueBooks(String stringSequence, Character bookEnd){                                       // 10 задание
    ...
    public static void main(String[] args) {
        System.out.println("1. " + composition(10,7, "hello my name is Bessie and this is my essay"));
